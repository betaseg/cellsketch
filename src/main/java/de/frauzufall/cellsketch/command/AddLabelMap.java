...
@Plugin(type = Command.class,
		menuPath = "Analyze>" + CellProject.appName + ">Add labels", headless = true)
public class AddLabelMap extends CommandColoredEntityWithFilamentConnection {

	@Parameter(label = "Labels (as labelmap)")
	@Option(name = "--input")
	private File input;
	...
	public void run() {
		...
		String progressName = "Adding labelmap from " + input;
		try {
			Double threshold = null;
			project.startProgress(progressName);
			if(analyzeConnectionFilamentEnds) threshold = thresholdConnectionFilamentEnds;
			project.addLabelMapItem(input, name, ARGBType.rgba(color.getRed(), color.getGreen(), color.getBlue(), color.getAlpha()), threshold, scaleX, scaleY, scaleZ);
