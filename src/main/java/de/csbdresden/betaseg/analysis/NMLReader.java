...
public class NMLReader {
	...
	public static RandomAccessibleInterval<RealType<?>> readIndexed(File xmlFile, double scale, int radius) throws NMLReaderIOException, DataConversionException {
		...
		List<List<Pair<Point, Point>>> microtubules = new ArrayList<>();

		List<Element> things = root.getChildren("thing");
		things.forEach(thing -> {
			List<Pair<Point, Point>> pointList = toPointList(thing, scale);
			if(pointList != null) microtubules.add(pointList);
		});
	...
	public static RandomAccessibleInterval<RealType<?>> readOnlyStraightLines(File xmlFile, double scale, int radius) throws NMLReaderIOException, DataConversionException {
		...
		for (List<Pair<Point, Point>> microtubule : microtubules) {
			if(microtubule.size() < 4) {
				for (Pair<Point, Point> line : microtubule) {
					drawLine(output, line.getA(), line.getB(), radius, 255);
				}
			}
		}
	...
	public static List<List<Pair<Point, Point>>> toPoints(File xmlFile, double scale) throws NMLReaderIOException, DataConversionException {
	...
	public static void drawLine(RandomAccessibleInterval image, Point p1, Point p2, int radius, int val) {
		...
		if ((l >= m) && (l >= n)) {
			...
			err_2 = dz2 - l;
			for (i = 0; i < l; i++) {
				DrawingUtil.drawPoint(image, point, radius, val);
				if (err_1 > 0) {
					point[1] += y_inc;
					err_1 -= dx2;
				}
